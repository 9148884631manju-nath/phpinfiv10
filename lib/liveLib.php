<?php ini_set('session.save_path', 'stemp'); session_start();
class php10{
	...
	public function meta($t,$f,$v,$ar)
	{
		...
		else
		{
			...
			{
				$rs.=" Pleae Set ".$t." Details for Page ".$v." in SEO File seo.json";
			}
		}
		return $rs;
	}
	public function value_set($k,$v,$p,$c)
	{
		...
		if(!isset($v->$p))
					{
						$pag=" Please Set [".$p."] Page Title in SEO File seo.json ";
					}
					else
					{
						$pag=$v->$p;
					...
					if(!isset($v->$c))
					{
						$cont=" Please Set [".$c."] Content Title in SEO File seo.json ";
					}
					else
					{
						$cont=$v->$c;
	...
	public function posted_values($t,$p,$d,$ad)
	{
		...
		switch($t)
		{
			...
			case "sessiontojson": $ar = $_SESSION[$p];if(is_array($ar) and count($ar)>0){$rs=htmlentities(json_encode($ar));}else{$rs=$d;} break;
	...
	{ 
		...
		case "congrad": 
			...
			{
				$spl = explode(" ",$sp[$j]);
				if($spl[0]=="transparent"){$col='transparent '.$spl[1];}else{$col=$spl[0]." ".$spl[1];}
				$cls.=$col.",";
			} 
				...
				switch($data_display)
				{
					case "html_xml":
						...
						$xml.='<data>';					
							...
							$xml.='</data>';
							//echo writefile('data.xml','Download XML File Here',$xml)."<hr/>";
					...
					case "csv":
						if(is_array($res)){
							...
							$this->exportcsv('temp\csvdata.csv','',$data);
							echo $this->read_file('temp\csvdata.csv');				
						}else
						{}					
					break;
					case "html_csv":
						if(is_array($res)){
							...
							$this->exportcsv('temp\csvdata.csv','',$data);
							echo $this->read_csv_html_file('temp\csvdata.csv');				
						}else
						{}					
					break;
					case "table_data":
							...
							if($paging==""){}else
								{
									...
									$jxb= $this->s_replace($sqlerr,$res,$jeb);
									$jxd=$this->jdec($jxb);
									$this->inblock($jxd,'');
						}
					break;
		...
		switch($recordtype)
		{
			case "updaterecord":
				...
				if($run=="yes")
					{
						$old_data[$aid] = $data;
						$replacedata = $old_data[$aid];					
						$contj= json_encode($old_data);
						$fo = fopen($ofle,"w");
						fwrite($fo,$contj);
						fclose($fo);
						if($redir==""){echo $message;}else{
